    ...
    def reward(self, value, board, jugueUltimo):
        if self.last_move:
            if not jugueUltimo:
                ...
                possible_moves = available_moves(board_from_tuple(self.last_board))

            self.learn(self.last_board, self.last_move, value, tuple_from_board(board), possible_moves)

    def learn(self, state, action, reward, result_state, possible_moves, jugueUltimo):
        prev = self.getQ(state, action)
        if len(possible_moves) > 0:
            if jugueUltimo:
                maxqnew = max([self.getQ(result_state, a) for a in possible_moves ])
            else:
                maxqnew = max([self.getQ(state, a) for a in possible_moves ])                
        else: maxqnew = 0
